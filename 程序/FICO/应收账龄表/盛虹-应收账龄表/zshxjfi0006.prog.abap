...
*变更记录
*修改请求号    修改日期    修改人   修改描述
*DEVK912013    2019-11-20   HANDYXH    创建程序
***************************************************
REPORT zshxjfi0006.
...
*         vptnr         TYPE ztcrm_kunvp-vptnr, "业务员编号
*         vptnt         TYPE but000-bu_sort1, "业务员
...
*         invoicenumber TYPE ztjs_feedback-invoicenumber,
         zmax          TYPE c,
         zmin          TYPE c,
         zmon(2)       TYPE c,
         clr(4)        TYPE c,
...
DATA: it_head  TYPE STANDARD TABLE OF x_head,   "输出汇总数据
      ...
      wa_head1 TYPE x_head,                     "汇总工作区
      wa_head2 TYPE x_head.                     "汇总工作区
...
DATA: it_item  TYPE STANDARD TABLE OF x_head,    "汇总模式双击行显示的明细内表
      ...
      wa_item1 TYPE x_head,
      wa_item2 TYPE x_head.
...
PARAMETERS:
p_kunnr AS CHECKBOX DEFAULT c_x.
...
SELECTION-SCREEN BEGIN OF LINE.
PARAMETERS:
p_lifnr AS CHECKBOX MODIF ID oo.
SELECTION-SCREEN COMMENT 2(6) FOR FIELD s_lifnr.
SELECT-OPTIONS:
s_lifnr FOR lfa1-lifnr MODIF ID oo.
SELECTION-SCREEN END OF LINE.
SELECTION-SCREEN BEGIN OF LINE.
PARAMETERS:
p_hkont AS CHECKBOX  MODIF ID oo.
SELECTION-SCREEN COMMENT 2(6) FOR FIELD s_hkont.
SELECT-OPTIONS:
s_hkont FOR skb1-saknr  MODIF ID oo.
SELECTION-SCREEN END OF LINE.
...
SELECTION-SCREEN BEGIN OF LINE.
PARAMETERS c_3 TYPE c AS CHECKBOX MODIF ID md2.               "明细模式
SELECTION-SCREEN COMMENT (8) mytext MODIF ID md2.
SELECTION-SCREEN END OF LINE.
...
*PARAMETERS: c_3 TYPE c as CHECKBOX MODIF ID md2.                                        "明细汇总模式
...
*  DATA : lt_ztcrm TYPE TABLE OF ztcrm_kunvp2.
*  DATA: lw_ztcrm TYPE  ztcrm_kunvp2.

  DATA: lt_but000 TYPE TABLE OF but000.
  DATA:lw_but000 TYPE but000.
...
*取事务代码
    ...
    "取业务员编号\业务员
*    SELECT *
*       INTO CORRESPONDING FIELDS OF TABLE lt_ztcrm
*       FROM ztcrm_kunvp2
*       FOR ALL ENTRIES IN it_acc_cus
*       WHERE kunnr EQ it_acc_cus-kunnr.
*
*    SELECT *
*      APPENDING CORRESPONDING FIELDS OF TABLE lt_ztcrm
*       FROM ztcrm_kunvp
*       FOR ALL ENTRIES IN it_acc_cus
*       WHERE kunnr EQ it_acc_cus-kunnr.

*    SORT lt_ztcrm BY kunnr.
*    DELETE ADJACENT DUPLICATES FROM lt_ztcrm.
...
*       FROM but000
*      FOR ALL ENTRIES IN lt_ztcrm
*      WHERE partner = lt_ztcrm-vptnr.
...
*取业务员编号
*      IF wa_head-kunnr NE ''.
*        READ TABLE lt_ztcrm INTO lw_ztcrm WITH KEY kunnr = wa_head-kunnr.
*        IF sy-subrc EQ 0.
*          wa_head-vptnr = lw_ztcrm-vptnr.
*
*          READ TABLE lt_but000 INTO lw_but000 WITH KEY partner = wa_head-vptnr.
*          IF sy-subrc EQ 0.
*            wa_head-vptnt = lw_but000-name_org1.
*          ENDIF.
*        ENDIF.
*
*      ENDIF.
...
*币种
      ...
      ENDIF.
*2019-4-1 BY XBW金税发票号
      SELECT SINGLE awkey INTO wa_head-awkey FROM bkpf WHERE bukrs = wa_acc_cus-bukrs AND belnr = wa_acc_cus-belnr AND gjahr = wa_acc_cus-gjahr AND blart = 'RV'.
*      SELECT SINGLE invoicenumber INTO wa_head-invoicenumber FROM ztjs_feedback WHERE  vbeln = wa_head-awkey AND bukrs =   wa_acc_cus-bukrs.
      wa_head-znf = wa_acc_cus-dudat+0(4).
      wa_head-zyf = wa_acc_cus-dudat+4(2).
...
*明细模式显示
  IF r_1 EQ 'X'.
    ...
    USING:  'KSBM'  text-051 'X' '' '' ,  "客商编码
            ...
            'AWKEY' text-089 '' '' '' ,  "销售发票号
*            'INVOICENUMBER' text-092 '' '' '' ,  "销售发票号
            'GUOJ'  text-055 '' '' '' ,      "国家

*            'VPTNR'  text-087 '' '' '' ,      "业务员编码
*            'VPTNT'  text-088 '' '' '' ,      "业务员
...
*IF  r_1 EQ 'X'.
*  lwa_fieldcat-EMPHASIZE     = P_COLOR.
*ENDIF.
...
*        lw_huizong_lrzx-vptnr =  wa_huizong-vptnr ."业务员编号
*        lw_huizong_lrzx-vptnt =  wa_huizong-vptnt ."业务员
...
*        lw_huizong_ywfw-vptnr = wa_huizong-vptnr ."业务员编号
*        lw_huizong_ywfw-vptnt = wa_huizong-vptnt ."业务员
...
*        lw_huizong_hebing-vptnr =    wa_huizong-vptnr ."业务员编号
*        lw_huizong_hebing-vptnt =    wa_huizong-vptnt ."业务员
...
*        lw_huizong_kesh-vptnr =  wa_huizong-vptnr ."业务员编号
*        lw_huizong_kesh-vptnt =  wa_huizong-vptnt ."业务员
...
*做金额合计
    ...
    "业务员
*    LOOP AT it_item ASSIGNING <f1>.
*
**      CLEAR:<f1>-vptnr,<f1>-vptnt.
*
*      IF <f1>-blart = 'YS'.
*        SELECT SINGLE xref1 INTO <f1>-vptnr FROM ztfi_ckgl_001
*          WHERE bukrs = <f1>-bukrs AND   belnr = <f1>-belnr  AND gjahr = <f1>-gjahr.
*        IF <f1>-vptnr IS NOT INITIAL.
*
*
*          CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
*            EXPORTING
...
*
*
*          SELECT SINGLE name_org1 INTO <f1>-vptnt  FROM but000
*            WHERE partner = <f1>-vptnr   .
*        ENDIF.
*      ELSEIF <f1>-blart = 'RV' .
*        SELECT SINGLE kunnr INTO <f1>-vptnr FROM bkpf
*             INNER JOIN vbrp ON vbrp~vbeln = bkpf~awkey
*          INNER JOIN vbpa ON vbpa~vbeln = vbrp~aubel
*
*        WHERE vbpa~parvw = 'Z1'
*           AND bkpf~bukrs = <f1>-bukrs AND bkpf~belnr = <f1>-belnr  AND bkpf~gjahr = <f1>-gjahr.
*
*        IF <f1>-vptnr IS NOT INITIAL.
*          CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
*            EXPORTING
...
*        ENDIF.
*      ELSEIF <f1>-blart = 'UE' .
*        SELECT SINGLE xref1 INTO <f1>-vptnr FROM bseg
*          WHERE  bukrs = <f1>-bukrs AND   belnr = <f1>-belnr  AND gjahr = <f1>-gjahr AND bseg~shkzg = 'H'.
*        IF <f1>-vptnr IS NOT INITIAL.
*          CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
*            EXPORTING
*              input  = <f1>-vptnr
*            IMPORTING
*              output = <f1>-vptnr.
*          SELECT SINGLE name1 INTO <f1>-vptnt  FROM kna1
*            WHERE kunnr = <f1>-vptnr   .
*        ENDIF.
*
*      ELSEIF <f1>-blart = 'SA' .
*        SELECT SINGLE vptnr INTO <f1>-vptnr FROM bseg
*         WHERE  bukrs = <f1>-bukrs AND   belnr = <f1>-belnr  AND gjahr = <f1>-gjahr AND bseg~buzei = <f1>-buzei.
*
*        IF <f1>-vptnr IS NOT INITIAL.
*          CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
*            EXPORTING
...
FORM f_field_item .
  ...
  CLEAR it_fieldcat.
  PERFORM f_fieldcat_set2 TABLES it_fieldcat
  USING:  'KSBM'  text-051 'X' '' ,  "客商编码
          'KSMS'  text-052 'X' '' ,  "客商描述
          'BUKRS' text-003 'X' '' ,  "公司代码
          'BUTXT' text-057 'X' '' ,  "公司代码描述
...
*            'VPTNT'  text-088 '' '' ,      "业务员
          ...
          'SGTXT' text-067 '' '' ,  "项目文本
          'KSYE ' text-073 '' '' , "客商余额
          'WDQJE' text-074 '' '' ,  "未到期余额
          'ZLJE'  text-075 '' '' ,  "账龄金额
          'WAERS' text-076 '' '' ,  "币种
          'XTYPE' text-077 '' '' .       "账龄类型

  IF p_date1 NE ''.
    PERFORM f_fieldcat_set2 TABLES it_fieldcat USING 'DMBT1' lv_dmbt1 ''  '' .
    CLEAR lv_dmbt9.
    CONCATENATE:'账龄大于等于' p_dy1 INTO lv_dmbt9.
  ENDIF.
  IF p_date2 NE ''.
    PERFORM f_fieldcat_set2 TABLES it_fieldcat USING 'DMBT2' lv_dmbt2 ''  '' .
    CLEAR lv_dmbt9.
    CONCATENATE:'账龄大于等于' p_dy2 INTO lv_dmbt9.
  ENDIF.
  IF p_date3 NE ''.
    PERFORM f_fieldcat_set2 TABLES it_fieldcat USING 'DMBT3' lv_dmbt3 ''  '' .
    CLEAR lv_dmbt9.
    CONCATENATE:'账龄大于等于' p_dy3 INTO lv_dmbt9.
  ENDIF.
  IF p_date4 NE ''.
    PERFORM f_fieldcat_set2 TABLES it_fieldcat USING 'DMBT4' lv_dmbt4 ''  '' .
    CLEAR lv_dmbt9.
    CONCATENATE:'账龄大于等于' p_dy4 INTO lv_dmbt9.
  ENDIF.
  IF p_date5 NE ''.
    PERFORM f_fieldcat_set2 TABLES it_fieldcat USING 'DMBT5' lv_dmbt5 ''  '' .
    CLEAR lv_dmbt9.
    CONCATENATE:'账龄大于等于' p_dy5 INTO lv_dmbt9.
  ENDIF.
  IF p_date6 NE ''.
    PERFORM f_fieldcat_set2 TABLES it_fieldcat USING 'DMBT6' lv_dmbt6 ''  '' .
    CLEAR lv_dmbt9.
    CONCATENATE:'账龄大于等于' p_dy6 INTO lv_dmbt9.
  ENDIF.
  IF p_date7 NE ''.
    PERFORM f_fieldcat_set2 TABLES it_fieldcat USING 'DMBT7' lv_dmbt7 ''  '' .
    CLEAR lv_dmbt9.
    CONCATENATE:'账龄大于等于' p_dy7 INTO lv_dmbt9.
  ENDIF.
  IF p_date8 NE ''.
    PERFORM f_fieldcat_set2 TABLES it_fieldcat USING 'DMBT8' lv_dmbt8 ''  '' .
    CLEAR lv_dmbt9.
    CONCATENATE:'账龄大于等于' p_dy8 INTO lv_dmbt9.
  ENDIF.

  PERFORM f_fieldcat_set2 TABLES it_fieldcat USING 'DMBT9' lv_dmbt9 ''  '' .
...
**排序 给每月最大行进行标记
  ...
  SORT it_head DESCENDING BY gjahr dudat belnr.
  READ TABLE it_head INTO wa_head INDEX 1.
  z1 = wa_head-dudat+4(2).
  CLEAR wa_head.
  LOOP AT it_head INTO wa_head.
    wa_head-zmon = wa_head-dudat+4(2).
    z2 = wa_head-dudat+4(2).
    IF z2 <> z1.
      wa_head-zmax = 'X'.
      z1 = z2.
    ENDIF.
  ...
  READ TABLE it_head INTO wa_head INDEX 1.
  wa_head-zmax = 'X'.
  MODIFY it_head FROM wa_head INDEX 1.

  SORT it_head  BY gjahr dudat belnr.
  READ TABLE it_head INTO wa_head INDEX 1.
*  WA_HEAD-ZMIN = 'X'.
  z1 = wa_head-dudat+4(2).
*  MODIFY TABLE IT_HEAD FROM WA_HEAD.
  CLEAR wa_head.
  LOOP AT it_head INTO wa_head.
    wa_head-zmon = wa_head-dudat+4(2).
    z2 = wa_head-dudat+4(2).
...
*2019-4-2当月合计 降序
  DATA zlj TYPE bsik-dmbtr.
  DATA zlj2 TYPE bsik-dmbtr.
  DATA: str1(20) TYPE c,
        str2(20) TYPE c,
        str3(20) TYPE c,
        str4(20) TYPE c.
...
*      STR4 = '年'.
*      WA_HEAD1-KSYE = ZLJ.
*      WA_HEAD1-ZNF = WA_HEAD2-ZNF.
*      WA_HEAD1-ZYF = WA_HEAD2-ZYF.
*      WA_HEAD1-ZHXM = 'X'.
*      WA_HEAD1-CLR = 'C500'.
*      CONCATENATE STR1 STR4 STR2 STR3 INTO WA_HEAD1-SGTXT .
*      INSERT WA_HEAD1 INTO IT_HEAD.
*      CLEAR: STR1, STR2, STR3, STR4.
*      CLEAR WA_HEAD1.
*      CLEAR ZLJ.
*    ENDIF.
*
*    ZLJ = WA_HEAD-KSYE + ZLJ.
*
*    IF WA_HEAD-ZMIN = 'X' .
*      WA_HEAD2-ZNF = WA_HEAD-DUDAT+0(4).
*      WA_HEAD2-ZYF = WA_HEAD-DUDAT+4(2).
*      ZNF3 =  WA_HEAD2-ZNF.
*      ZYF3 =  WA_HEAD2-ZYF.
*      ZLJ2 = ZLJ.
*    ENDIF.
...
*      WA_HEAD1-CLR = 'C500'.
*      CONCATENATE STR1 STR4 STR2 STR3 INTO WA_HEAD1-SGTXT .
*      APPEND WA_HEAD1 TO IT_HEAD.

*2019-4-2 当月合计 升序
  LOOP AT it_head INTO wa_head.
    IF zlj IS NOT INITIAL.
      IF wa_head-zmin = 'X'.
        str1 = wa_head2-znf.
        str2 = wa_head2-zyf.
        str3 = '月合计'.
...
*2019-4-2月末累计
  CLEAR: wa_head2,wa_head,str1,str2.
  LOOP AT it_head INTO wa_head WHERE zhxm = 'X'.
    wa_head2-zyf = wa_head-zyf.
    wa_head2-znf = wa_head-znf.
    wa_head2-clr = 'C500'.
    str1 = wa_head-znf.
    str2 = wa_head-zyf.
    str3 = '月末累计'.
    str4 = '年'.
    CONCATENATE str1 str4 str2 str3 INTO wa_head2-sgtxt .
    wa_head2-ksye = wa_head-ksye + wa_head2-ksye.
    INSERT wa_head2 INTO it_head.
  ENDLOOP.
...
*  LOOP AT IT_HEAD INTO WA_HEAD WHERE ZHXM = 'X'.
**    IF WA_HEAD-ZHXM = 'X'.
*       IF WA_HEAD-ZYF = '11' OR WA_HEAD-ZYF = '12'.
*          ZYF2 = WA_HEAD-ZYF - 1.
*          READ TABLE IT_HEAD INTO WA_HEAD1 WITH KEY ZNF  = WA_HEAD-ZNF
*                                                    ZYF  = ZYF2
*                                                    ZHXM = 'X'.
*          WA_HEAD2-KSYE = WA_HEAD-KSYE + WA_HEAD1-KSYE.
*          WA_HEAD2-ZYF = WA_HEAD-ZYF.
*          WA_HEAD2-ZNF = WA_HEAD-ZNF.
*          WA_HEAD2-CLR = 'C500'.
*          STR1 = WA_HEAD-ZNF.
...
*          CONCATENATE STR1 STR4 STR2 STR3 INTO WA_HEAD2-SGTXT .
*          INSERT WA_HEAD2 INTO IT_HEAD.
*          CLEAR: WA_HEAD1, WA_HEAD2, ZYF2.
*       ELSEIF WA_HEAD-ZYF = '01'.
*          ZYF2 = '12'.
*          ZNF2 = WA_HEAD-ZNF - 1.
*          READ TABLE IT_HEAD INTO WA_HEAD1 WITH KEY  ZNF  = ZNF2
*                                                    ZYF  = ZYF2
*                                                    ZHXM = 'X'.
*          WA_HEAD2-KSYE = WA_HEAD-KSYE + WA_HEAD1-KSYE.
*          WA_HEAD2-ZYF = WA_HEAD-ZYF.
...
*          CONCATENATE STR1 STR4 STR2 STR3 INTO WA_HEAD2-SGTXT .
*          INSERT WA_HEAD2 INTO IT_HEAD.
*          CLEAR: WA_HEAD1, WA_HEAD2, ZYF2,ZNF2.
*       ELSE.
*          ZYF2 = WA_HEAD-ZYF - 1.
*          CONCATENATE STR6 ZYF2 INTO ZYF2.
*          READ TABLE IT_HEAD INTO WA_HEAD1 WITH KEY ZNF  = WA_HEAD-ZNF
*                                                    ZYF  = ZYF2
*                                                    ZHXM = 'X'.
*          WA_HEAD2-KSYE = WA_HEAD-KSYE + WA_HEAD1-KSYE.
...
*2019-4-8处理单机事件中的数据汇总
  ...
  SORT it_item DESCENDING BY gjahr dudat belnr.
  READ TABLE it_item INTO wa_item INDEX 1.
  z1 = wa_item-dudat+4(2).
  CLEAR wa_item.
  LOOP AT it_item INTO wa_item.
    wa_item-zmon = wa_item-dudat+4(2).
    z2 = wa_item-dudat+4(2).
    IF z2 <> z1.
      wa_item-zmax = 'X'.
      z1 = z2.
    ENDIF.
  ...
  SORT it_item  BY gjahr dudat belnr.
  READ TABLE it_item INTO wa_item INDEX 1.
*  WA_HEAD-ZMIN = 'X'.
  z1 = wa_item-dudat+4(2).
*  MODIFY TABLE IT_HEAD FROM WA_HEAD.
...
*  IF ZLJ IS NOT INITIAL.
    IF wa_item-zmin = 'X'.
      ...
      str4 = '年'.
      wa_item1-ksye = zlj.
      wa_item1-znf = wa_item2-znf.
      wa_item1-zyf = wa_item2-zyf.
      wa_item1-zhxm = 'X'.
      wa_item1-clr = 'C500'.
      CONCATENATE str1 str4 str2 str3 INTO wa_item1-sgtxt .
      INSERT wa_item1 INTO it_item.
      CLEAR: str1, str2, str3, str4.
      CLEAR wa_item1.
      CLEAR zlj.
    ENDIF.
*  ENDIF.
    zlj = wa_item-ksye + zlj.

    IF wa_item-zmax = 'X' .
      wa_item2-znf = wa_item-dudat+0(4).
      wa_item2-zyf = wa_item-dudat+4(2).
      znf3 =  wa_item2-znf.
      zyf3 =  wa_item2-zyf.
      zlj2 = zlj.
    ENDIF.
  ...
  CLEAR: wa_item2,wa_item,str1,str2.


  DELETE it_item INDEX 1.

*2019-4-9年合计修改
*     LOOP AT IT_ITEM INTO WA_ITEM WHERE ZHXM = 'X'.
*       WA_ITEM2-ZYF = WA_ITEM-ZYF.
*       WA_ITEM2-ZNF = WA_ITEM-ZNF.
*       WA_ITEM2-CLR = 'C500'.
*       STR1 = WA_ITEM-ZNF.
*       STR2 = WA_ITEM-ZYF.
*       STR3 = '月末累计'.
*       STR4 = '年'.
*       CONCATENATE STR1 STR4 STR2 STR3 INTO WA_ITEM2-SGTXT .
*       WA_ITEM2-KSYE = WA_ITEM-KSYE + WA_ITEM2-KSYE.
*       INSERT WA_ITEM2 INTO IT_ITEM .
*     ENDLOOP.
...
FORM f_fieldcat_set2  TABLES pt_fieldcat2 TYPE slis_t_fieldcat_alv
                      USING p_field     TYPE char30
                            p_text      TYPE char40
                            p_fixed     TYPE c
                            p_hotspot   TYPE c.
...
*  lwa_fieldcat-do_sum     = p_dosum.
  ...
  APPEND lwa_fieldcat TO pt_fieldcat2.
  CLEAR lwa_fieldcat.
ENDFORM.
