    ...
    LOOP AT it_l_data INTO wa_l_data.
      ...
      IF sy-subrc = 0.
        ...
        PERFORM frm_deal_date USING wa_lc_data-field06
                              CHANGING lv_date.
        gs_data-zfbdt = wa_lc_data-field06.
...
*        ASSIGN COMPONENT ls_fieldcat-fieldname OF STRUCTURE ls_data TO <fs_field>.
*        IF <fs_field> IS ASSIGNED.
*          IF <fs_field> IS INITIAL.
*            MOVE-CORRESPONDING ls_data TO ls_log.
*            ls_log-ztype = 'E'.
*            CONCATENATE '字段' ls_fieldcat-reptext '不能为空！' INTO ls_log-zmsg.
...
*      ENDLOOP.
*
*      CHECK ls_log-ztype IS INITIAL.
*
*      DATA:lv_zuonr TYPE bseg-zuonr.
*
*      "校验字段是否已经传输过
*      SELECT SINGLE zuonr INTO lv_zuonr FROM zzt_dianzihp_log WHERE zuonr = ls_data-zuonr AND ztype = 'E'.
*      IF sy-subrc = 0.
*        MOVE-CORRESPONDING ls_data TO ls_log.
*        ls_log-ztype = 'E'.
*        ls_log-zmsg = '该汇票已经传输过！'.
*        error_flag = 'X'.
*        APPEND ls_log TO lt_log.
*      ENDIF.
*      APPEND ls_data TO lt_data_tmp.
*      CLEAR ls_data.
*      "如果一个公司代码有一条数据出现错误 则不进行操作
*      AT END OF bukrs.
*        IF error_flag IS INITIAL.
*          APPEND LINES OF lt_data_tmp TO gt_data.
*          CLEAR lt_data_tmp.
*        ENDIF.
*        CLEAR error_flag.
*      ENDAT.
...
FORM frm_send_email  TABLES   p_log STRUCTURE zzt_dianzihp_log.
  ...
  LOOP AT p_log.
    ...
    CONCATENATE:  p_log-bukrs gc_tab
                  p_log-lifnr gc_tab
                  p_log-zuonr gc_tab
                  p_log-cpudt gc_tab
                  p_log-cputm gc_tab
